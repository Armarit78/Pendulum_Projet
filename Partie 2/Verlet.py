...
def F(Y, l1, l2, m1, m2):
    ...
    # Calcul de s1(Y) et s2(Y) selon les formules de la fiche
    s1 = (-m2 * l1 * dth1 ** 2 * np.sin(th1 - th2) * np.cos(th1 - th2) + g * m2 * np.sin(th2) * np.cos(th1 - th2) - m2 * l2 * dth2 ** 2 * np.sin(th1 - th2) - g * (m1 + m2) * np.sin(th1)) / (l1 * (m1 + m2) - m2 * l1 * np.cos(th1 - th2) ** 2)
    s2 = (m2 * l2 * dth2 ** 2 * np.sin(th1 - th2) * np.cos(th1 - th2) + g * (m1 + m2) * np.sin(th1) * np.cos(th1 - th2) + (m1 + m2) * l1 * dth1 ** 2 * np.sin(th1 - th2) - g * (m1 + m2) * np.sin(th2)) / (l2 * (m1 + m2) - m2 * l2 * np.cos(th1 - th2) ** 2)

    return np.array([dth1, dth2, s1, s2])


def verlet_method(Y, l1, l2, m1, m2, dt, T):
    N = int(T / dt)
    ...
    for i in range(N - 1):
        th1, th2, dth1, dth2 = sol[:, i]
        _, _, s1, s2 = F(sol[:, i], l1, l2, m1, m2)

        # Mise à jour des positions
        th1_next = th1 + dt * dth1 + 0.5 * dt**2 * s1
        th2_next = th2 + dt * dth2 + 0.5 * dt**2 * s2

        # Calcul intermédiaire avec les nouvelles positions
        _, _, s1_new, s2_new = F([th1_next, th2_next, dth1, dth2], l1, l2, m1, m2)

        # Mise à jour des vitesses
        dth1_next = dth1 + 0.5 * dt * (s1 + s1_new)
        dth2_next = dth2 + 0.5 * dt * (s2 + s2_new)

        sol[:, i + 1] = [th1_next, th2_next, dth1_next, dth2_next]
...
temps, sol = verlet_method(Y0, l1, l2, m1, m2, dt, temps_aquis)
...
angle_plot(temps, sol, filename=os.path.join(repertoire_script_courant, 'angle_plot_4.png'))
angular_velocity_plot(temps, sol, filename=os.path.join(repertoire_script_courant, 'angular_velocity_plot_4.png'))
plot_trajectories(temps, sol, filename=os.path.join(repertoire_script_courant, 'trajectories_4.png'))

def save_animation_once(ani, filename='double_pendulum_2sec_4.mp4', fps=200):
...
#save_animation_once(ani, os.path.join(repertoire_script_courant, 'double_pendulum_2sec_4.mp4'))
...
def energy_evolution_plot(temps, Ec_values, Ep_values, Et_values, filename):
    ...
    plt.title("Evolution de l'énergie totale du double pendule")
    plt.legend()
    plt.grid(True)
    plt.savefig(filename)
    plt.close()

energy_evolution_plot(temps, Ec_values, Ep_values, Et_values, filename=os.path.join(repertoire_script_courant, 'energy_evolution_4.png'))

#Check si energie conservée
# Energy is conserved if the standard deviation of Et is small
energy_conserved = np.std(Et_values) < 1e-5
energy_conservation_status = 'conservée' if energy_conserved else 'non conservée'
print(f"L'energie du double pendule est {energy_conservation_status}.")
print(f"La différence d'énergie mécanique est de {np.std(Et_values)}")
